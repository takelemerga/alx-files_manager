...
class FilesController {
  static async postUpload(req, res) {
    ...
    if (idParent !== 0) {
      const parentFile = await (await DBClient.filesCollection('files')).findOne({ _id: ObjectId(redisToken) });
    ...
    if (['folder'].includes(fileType)) {
      await (await DBClient.filesCollection('files')).insertOne(dbFile);
  ...
  static async getShow(req, res) {
    ...
    const fileDocument = await (await DBClient.filesCollection('files')).findOne({ _id: ObjectId(idFile), userId: user._id });
  ...
  static async getIndex(req, res) {
    ...
    const files = await (await DBClient.filesCollection('files')).aggregate(aggregateData);
    const filesArray = [];
  ...
  static async putPublish(req, res) {
    ...
    let fileDocument = await (await DBClient.filesCollection('files')).findOne({ _id: ObjectId(idFile), userId: user._id });
    if (!fileDocument) return res.status(404).send({ error: 'Not found' });

    await (await DBClient.filesCollection('files')).update({ _id: ObjectId(idFile) }, { $set: { isPublic: true } });
    fileDocument = await (await DBClient.filesCollection('files')).findOne({ _id: ObjectId(idFile), userId: user._id });
  ...
  static async getFile(req, res) {
    ...
    const fileDocument = await (await DBClient.filesCollection('files')).findOne({ _id: ObjectId(idFile) });
    if (!fileDocument) return res.status(404).send({ error: 'Not found' });
    ...
    if (token) {
      const redisToken = await RedisClient.get(`auth_${token}`);
      if (redisToken) {
        user = await (await DBClient.usersCollection('users')).findOne({ _id: ObjectId(redisToken) });
